...
pub const Handle = struct {
    ...
    impl: struct {
        ...
        document_scope: analysis.DocumentScope = undefined,
        zir: Zir = undefined,
    ...
    pub fn getDocumentScope(self: *Handle) error{OutOfMemory}!analysis.DocumentScope {
    ...
    fn getDocumentScopeCold(self: *Handle) error{OutOfMemory}!analysis.DocumentScope {
        @setCold(true);
        ...
        while (true) {
            ...
            self.impl.document_scope = blk: {
                const tracy_zone_inner = tracy.traceNamed(@src(), "analysis.makeDocumentScope");
                defer tracy_zone_inner.end();

                var document_scope = try analysis.makeDocumentScope(self.impl.allocator, self.tree);
